...
class DataHandler(object):
    """ DataHandler is an abstract base class providing an interface for all subsequent
        ...
        This will replicate how a live strategy would function as current market data
        would be sent down the pipeline. Thus a historic and live system will be treated the same."""
    ...
    def update_bars(self):
        """ Pushes the latest bar to the latest symbol structure for all symbols in the list."""

        raise NotImplementedError("Should implement update_bars()")

class HistoricCSVDataHandler(DataHandler):
    """ HistoricCSVDataHandler is designed to read CSV files for each requested symbol from
        disk and provide an interface to obtain the 'latest' bar in a manner identical to
        a live trading interface."""

    def __init__(self, events, csv_dir, symbol_list):
        """ Initialises the historic data handler by requesting the location of
            the CSV files and list of symbols. It will be assumed that all files are of the form
    ...
    def open_convert_csv_files(self):
        """ Opens the CSV Files from the data directory, converting them into pandas
            DataFrames within a symbol dictionay."""

        comb_index = None
        for s in self.symbol_list:
            # Load the CSV file with no header informatio, indexed on date
            self.symbol_data[s] = pd.io.parsers.read_csv(os.path.join(self.csv_dir, '%s.csv' % s),
                                                        header = 0, index_col = 0,
                                                        names = ['datetime', 'open', 'high' ,'low', 'close', 'volume', 'open interest'])
    ...
    def get_new_bar(self, symbol):
        """ Returns the latest bar from the data feed as a tupel of
